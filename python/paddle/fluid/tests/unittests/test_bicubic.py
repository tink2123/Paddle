...
def bicubic_interp_np(input,
    ...
    print("in_h={},out_h={},ratio_h={}".format(in_h,out_h,ratio_h))
    if out_w > 1:
        if (align_corners):
            ...
            for i in range(batch_size):
                for j in range(channel):
                    ...
                    for ii in range(4):
                        ...
                        coefficients[ii] = cubic_interp1d(input[i,j,access_y,access_x_0],
                    ...
                    out[i, j, k, l] = cubic_interp1d(
                        ...
                        coefficients[3], y_t)
                    #print("[i,j,k,l]:{},out:{}".format([i,j,k,l],out[i,j,k,l]))
    if data_layout == "NHWC":
...
class TestBicubicInterpCase5(TestBicubicInterpOp):
    def init_test_case(self):
        ...
        self.out_w = 11
        self.scale = 0.
        self.out_size = np.array([5, 11]).astype("int32")
        self.align_corners = False
...
class TestBicubicInterpOpUint8(OpTest):
    def setUp(self):
        self.out_size = None
        self.actual_shape = None
        self.init_test_case()
