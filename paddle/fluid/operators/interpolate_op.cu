...
template <typename T>
__device__ __forceinline__ static T Kecubic_convolution1(
    T x,
    T A) {
...
template <typename T>
__device__ __forceinline__ static T Kecubic_convolution2(
    T x,
    T A) {
...
__device__ __forceinline__ static T Kecubic_interp(
    ...
    T t) {
  T coeffs[4];
  Keget_cubic_upsampling_coefficients<T>(coeffs, t);
...
__global__ void KeBicubicInterpFw(
  ...
  for (; tid < nthreads; tid += stride){
    ...
    T in_img_idx = align_corners
                       ...
                       : static_cast<int>(ratio_w * (out_img_idx + 0.5) - 0.5);
    in_img_idx = (in_img_idx > 0) ? in_img_idx : 0;
    int input_x = int(in_img_idx);
    const T x_t = in_img_idx - input_x;

    T coefficients[4];
    if (data_layout == DataLayout::kNCHW) {
      for (int k = 0; k < 1; k++){
        int access_y = max(min(input_y - 1 + k , int(in_img_h) - 1), 0);
        int access_x_0 = max(min(input_x - 1, int(in_img_w) -1), 0);
        int access_x_1 = max(min(input_x + 0, int(in_img_w) -1), 0);
        int access_x_2 = max(min(input_x + 1, int(in_img_w) -1), 0);
        int access_x_3 = max(min(input_x + 2, int(in_img_w) -1), 0);
        ...
        coefficients[k] = Kecubic_interp(*in_pos_0,*in_pos_1,*in_pos_2,*in_pos_3,x_t);
      }

      out[out_id_h * output_w + out_id_w] = static_cast<T>(Kecubic_interp(coefficients[0],
                                                                         coefficients[1],
                                                                         coefficients[2],
                                                                         coefficients[3],
                                                                         y_t));
      ...
      for (int k = 0; k < 1; k++){
        ...
        const T* in_pos_0 = &in[out_id_h * input_w + access_y * in_img_w * num_channels +
                                access_x_0 * num_channels + channel_id]; 
        const T* in_pos_1 = &in[out_id_h * input_w + access_y * in_img_w * num_channels +
                                access_x_1 * num_channels + channel_id]; 
        const T* in_pos_2 = &in[out_id_h * input_w + access_y * in_img_w * num_channels +
                                access_x_2 * num_channels + channel_id]; 
        const T* in_pos_3 = &in[out_id_h * input_w + access_y * in_img_w * num_channels +
  ...
  for (; tid < nthreads; tid += stride){
    ...
    for (int i = 0; i < 4; i++){
      for (int j = 0; j < 4; j++){
        ...
        int access_y = max(min(int(input_y - 1 + j),int(in_img_h -1)),0);
        int access_x = max(min(int(input_x - 1 + i),int(in_img_w -1)),0);
        ...
        platform::CudaAtomicAdd(
            &in_pos[0], (out_pos[0] * y_coeffs[i] * x_coeffs[j]));
...
static void Interpolate2DCUDAFwd(const framework::ExecutionContext& ctx,
  ...
  if ("nearest" == interp_method) {
    ...
    KeBicubicInterpFw<T><<<config.blocks, config.threads, 0,
                            ctx.cuda_device_context().stream()>>>(
        input_data, in_h, in_w, n, in_chw, output_data, out_h, out_w, n,
        out_chw, c, ratio_h, ratio_w, align_corners, data_layout);
  }
...
static void Interpolate2DCUDABwd(const framework::ExecutionContext& ctx,
  ...
  if ("nearest" == interp_method) {
    ...
    KeBicubicInterpBw<T><<<config.blocks, config.threads, 0,
                            ctx.cuda_device_context().stream()>>>(
        input_grad_data, in_h, in_w, n, in_chw, output_grad_data, out_h, out_w,
