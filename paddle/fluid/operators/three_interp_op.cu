...
template <typename T>
__global__ void KeThreeInterpFw(T* output, const T* input, const T* weight,
                                const int* idx, const int b, const int m,
...
template <typename T>
__global__ void KeThreeInterpBw(T* input_grad, const T* output_grad,
                                const T* weight, const int* idx, const int b,
                                const int m, const int c, const int n) {
  int nthreads = b * n * c;
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
  int stride = blockDim.x * gridDim.x;
  for (; tid < nthreads; tid += stride) {
...
REGISTER_OP_CUDA_KERNEL(three_interp, ops::ThreeInterpOpCUDAKernel<float>,
