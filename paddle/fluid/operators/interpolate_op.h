      ...
      for (int i = 0; i < n; i++) {    // loop for batches
        for (int j = 0; j < c; j++) {  // loop for channels
          ...
          for (int ii =0 ; ii < 4; ii++){
             int access_y = std::max(std::min(input_y - 1 + ii , in_h - 1), static_cast<int>(0));
             int access_x_0 = std::max(std::min(input_x - 1, in_w - 1), static_cast<int>(0));
             int access_x_1 = std::max(std::min(input_x + 0, in_w - 1), static_cast<int>(0));
             int access_x_2 = std::max(std::min(input_x + 1, in_w - 1), static_cast<int>(0));
             int access_x_3 = std::max(std::min(input_x + 2, in_w - 1), static_cast<int>(0));
             coefficients[ii] = cubic_interp<T>(
               input_t(i,j,access_y,access_x_0),
          ...
          // interp y direction
          if (data_layout == DataLayout::kNCHW){
          output_t(i,j,k,l) = cubic_interp<T>(
              coefficients[0],
              coefficients[1],
              coefficients[2],
              coefficients[3],
          ...
          output_t(i,k,l,j) = cubic_interp<T>(
              coefficients[0],
              coefficients[1],
              coefficients[2],
              coefficients[3],
...
template <typename T>
static void BicubicInterpolationGrad(const Tensor& output_grad, Tensor* input_grad,
                                  const float ratio_h, const float ratio_w,
                                  const int in_h, const int in_w, const int n,
                                  const int c, const int out_h, const int out_w,
                                  const bool align_corners,
        ...
        for (int j = 0; j < c; j++) {  // loop for channels
          ...
          for (int ii = 0 ; ii < 4; ii++){
            for (int jj = 0; jj < 4; jj++){
              int access_x = std::max(std::min(input_x - 1 + ii, in_w - 1), static_cast<int>(0));
              int access_y = std::max(std::min(input_y - 1 + jj, in_h - 1), static_cast<int>(0));

              if (data_layout == DataLayout::kNCHW) {
                T grad = output_grad_t(i, j, k, l);
                input_grad_t(i,j,access_y,access_x) += grad * y_coeffs[jj] * x_coeffs[ii];
              } else {
                T grad = output_grad_t(i, k, l, j);
                input_grad_t(i,access_y,access_x,j) += grad * y_coeffs[jj] * x_coeffs[ii];
              }
...
static void Interpolate2DCPUFwd(const framework::ExecutionContext& ctx,
  ...
  if ("bilinear" == interp_method) {
    ...
    BicubicInterpolation<T>(input, output, ratio_h, ratio_w, in_h, in_w, n, c, out_h,
                            out_w, align_corners, data_layout);
  }
}
...
static void Interpolate2DCPUBwd(const framework::ExecutionContext& ctx,
  ...
  if ("bilinear" == interp_method) {
    ...
    BicubicInterpolationGrad<T>(output_grad, input_grad, ratio_h, ratio_w,
                                in_h, in_w, n, c, out_h, out_w, align_corners, data_layout);
  }
}
