...
__global__ void QueryBall(int b, int n, int m, T radius, int nsample,
                          const T *points, const T *new_points, int *idx) {
...
class QueryBallOpCUDAKernel : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    QueryBall<<<batch_size, 256>>>(batch_size, n, m, radius, nsample, p_points,
                                   p_new_points, p_out_points);
  }
};
